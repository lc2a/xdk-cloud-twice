...
int deliverMessage(Client* c, MQTTString* topicName, MQTTMessage* message)
{
    ...
    if (rc ==XDK_RETCODE_FAILURE && c->defaultMessageHandler != NULL)
    {
        MessageData md;
        NewMessageData(&md, topicName, message);
...
int keepalive(Client* c)
{
    ...
    if (expired(&c->ping_timer))
    {
        if (!c->ping_outstanding)
        {
            ...
            if (len > 0 && (rc = sendPacket(c, len, &timer)) == XDK_RETCODE_SUCCESS) // send the ping packet
    ...
    switch (packet_type)
    {
        ...
        case PUBREC:
        {
            ...
            if (MQTTDeserialize_ack(&type, &dup, &mypacketid, c->readbuf, c->readbuf_size) != 1)
                ...
                rc =XDK_RETCODE_FAILURE;
            else if ((rc = sendPacket(c, len, timer)) != XDK_RETCODE_SUCCESS) // send the PUBREL packet
                rc =XDK_RETCODE_FAILURE; // there was a problem
            if (rc ==XDK_RETCODE_FAILURE)
                goto exit; // there was a problem
            break;
        }
...
int MQTTYield(Client* c, int timeout_ms)
{
    ...
    while (!expired(&timer))
    {
        if (cycle(c, &timer) ==XDK_RETCODE_FAILURE)
        {
            rc =XDK_RETCODE_FAILURE;
            break;
        }
...
int MQTTConnect(Client* c, MQTTPacket_connectData* options)
{
    ...
    if ((rc = sendPacket(c, len, &connect_timer)) != XDK_RETCODE_SUCCESS)  // send the connect packet
        goto exit; // there was a problem
...
int MQTTSubscribe(Client* c, const char* topicFilter, enum QoS qos, messageHandler messageHandler)
{
    ...
    if (len <= 0)
        goto exit;
    if ((rc = sendPacket(c, len, &timer)) != XDK_RETCODE_SUCCESS) // send the subscribe packet
        goto exit;             // there was a problem
