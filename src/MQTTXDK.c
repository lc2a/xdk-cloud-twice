...
/* This assumes you have updated the wifi firmware and flashed the root certificate */
#ifdef CERT_CHECK
#define SL_CA_FILE "/cert/relayr.der"
#define HOST "*.relayr.io"
#endif
...
int TLSConnectNetwork(Network *n, char* addr, int port, SlSockSecureFiles_t* certificates, unsigned char sec_method, unsigned int cipher, char server_verify) {
	...
	DEBUG_PRINT("Connecting using certificate validation");
	if (certificates != NULL) {
			retVal = sl_SetSockOpt(n->my_socket, SL_SOL_SOCKET, SL_SO_SECURE_FILES_CA_FILE_NAME, SL_CA_FILE, strlen(SL_CA_FILE));
			if(retVal < 0)
			{
				return retVal;
			}
